    ...
    setState(() {
      ...
      if (newMessage["isSender"] == true) {
        final optIndex = _messages.indexWhere((m) {
          ...
          if (mId.length <= 10) return false; // Not a temp ID

          final sameText =
              (m["text"] ?? '').toString().trim() ==
              (newMessage["text"] ?? '').toString().trim();
          bool sameSurvey = true;
          ...
          return sameText ||
              isUploadingImg ||
              (newMessage["survey"] != null && sameSurvey);
        });
        if (optIndex != -1) {
    ...
    try {
      ...
      if (imageId != null && _activeConversationId != null) {
        ...
        final success = await ApiService.sendMessage(
          ...
          type: mediaType,
          media: [
            {"url": imageId, "type": mediaType, "_id": imageId},
          ],
        );
        if (success) {
